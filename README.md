...
- **User detail page** — `/u/<id>` shows a registered user's public stats: avatar (or anonymous-mask fallback), `@handle`, country, join date, lifetime likes/dislikes cast as a compact stats block in the header, and a scrollable list of their 15 most recent opinions, newest first. Clickable from any `@user` mention in the app (profile detail, hover tooltip, stats leaderboard). Anonymous accounts have no detail page (404 with friendly "this person has left the building" message); the FE skips the link automatically when the reporter is anonymous or has been deleted.
