...
export function SourceLink({ profileId, host }: { profileId: string; host?: string | null }) {
  ...
  // transparency signal - users see where the link goes before clicking, even
  // though the full URL stays behind the /l/:id redirect. The hover tip repeats
  // it in full for the case where the chip has truncated it.
  return (
    <HoverTip label={host ? `${t.sourceLink}: ${host}` : t.sourceLink} className="inline-flex max-w-full min-w-0">
      <a
